...
Public Class FrmListaContribuyentes
    '#Region "IOpcion"
    ...
    Public Property Contribuyente() As Contribuyente
        ...
        Set(ByVal value As Contribuyente)
            If Not ListBindingSource.Count = 0 And value IsNot Nothing Then
                Dim t As Integer = 0
                For Each _Contribuyente As Contribuyente In Me.ListBindingSource.DataSource
                    If _Contribuyente.Entida_Codigo = value.Entida_Codigo Then
...
#Region "Eventos"
    Private Sub FrmListaContribuyentes_Abrir(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Abrir
        Abrir_Mantenimiento()
    End Sub

    Private Sub FrmListaContribuyentes_Seleccionar(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Seleccionar
        DialogResult = Windows.Forms.DialogResult.OK
        Me.Close()
    End Sub

    Private Sub FrmListaContribuyentes_Agregar(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Agregar
    ...
    Sub Abrir_Mantenimiento()
        If ListBindingSource.Current Is Nothing Then
            Exit Sub
        End If
        Dim f As New FrmMantenimientoContribuyente(Sistema, Restriccion)
        f.Contribuyentes = ListBindingSource
        f.ShowDialog()
    ...
    Private Sub FrmListaContribuyentes_Buscar(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles Me.Buscar
        Llenar_datos(Me.combobuscar.Text)
    End Sub

    Private Sub FrmListaContribuyentes_CancelarBuscar(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.CancelarBuscar, Me.Refrescar
        Llenar_datos()
    End Sub

    Private Sub FrmListaContribuyentes_Eliminar(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Eliminar
        If mContribuyentes Is Nothing OrElse ListBindingSource.Current Is Nothing Then
            Exit Sub
        End If
        Dim mContribuyente As Contribuyente
        mContribuyente = ListBindingSource.Current
        If mContribuyente.Eliminar() AndAlso ListBindingSource.Current IsNot Nothing Then
...
#Region "Llenardatos"
    Private WithEvents mContribuyentes As ContribuyenteList = Nothing

    Sub Llenar_datos(Optional ByVal _filtro As String = "")
        MyBase.Titulo = "Contribuyentes"
        MyBase.AgregarLeyenda = "Agregar un nuevo Contribuyente"
        ...
        Me.ListBindingSource.DataSource = GetType(Incautacion.Reglas.Contribuyente)
        mContribuyentes = ContribuyenteList.ObtenerLista(Sistema.OperadorDatos, _filtro)
        Dim mitemssort As New Infoware.Reglas.SortedView(mContribuyentes)
        ListBindingSource.DataSource = mitemssort
        Me.DataGridView1.AutoDiscover()
    End Sub

    Private Sub mContribuyentes_AddingNew(ByVal sender As Object, ByVal e As System.ComponentModel.AddingNewEventArgs) Handles mContribuyentes.AddingNew
...
#Region "New"
    Public Sub New(ByVal _Sistema As Sistema, ByVal _Restriccion As Restriccion, ByVal _busqueda As Boolean, Optional ByVal _OpcionNuevo As Enumerados.EnumOpciones = Enumerados.EnumOpciones.Usuarios)
        ...
        FrmListaContribuyentes_Inicializar(Me, Nothing)
    End Sub
    ...
    Private Sub FrmListaContribuyentes_Inicializar(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Inicializar
        Llenar_datos()
    End Sub
#End Region
